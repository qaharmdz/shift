...
$.extend($.fn.dataTable.defaults, {
        ...
        buttons: [
            {
                extend: "colvis",
                text:
                    '<span uk-tooltip title="Columns visual">' +
                    shift.i18n.columns +
                    "</span>",
                background: false,
    ...
    language: {
        ...
        infoPostFix:
            '<a data-dtReload uk-tooltip title="' +
            shift.i18n.reload_data +
    ...
    return function (request, drawCallback, settings) {
        ...
        if (ajax) {
            ...
            if (requestStart < cacheLower) {
                requestStart =
                    requestStart - requestLength * (config.pages - 1);
...
$.fn.dataTable.Api.register("clearSearch()", function () {
    return this.iterator("table", function (settings) {
        ...
        for (; iCol < settings.aoPreSearchCols.length; iCol++) {
            if (typeof settings.aoPreSearchCols[iCol].search !== "undefined") {
                // set back to initial search
                settings.aoPreSearchCols[iCol].sSearch =
                    settings.aoPreSearchCols[iCol].search;
            } else {
                settings.aoPreSearchCols[iCol].sSearch = "";
            }
        }
...
// UIkit Pagination
// ================================================
$.fn.dataTable.ext.renderer.pageButton.uikit = function (
    settings,
    host,
    idx,
    buttons,
            ...
            if ($.isArray(button)) {
                ...
                switch (button) {
                    ...
                    case "next":
                        ...
                        btnClass =
                            button + (page < pages - 1 ? "" : " uk-disabled");
                        break;
                ...
                if (btnDisplay) {
                    ...
                    if (
                        btnClass === "uk-active" ||
                        btnClass.indexOf("uk-disabled") >= 0
                    ) {
                        node = node.append($("<span>").html(btnDisplay));
                    } else {
                        node = node.append(
                            $("<a>", { href: "#" }).html(btnDisplay)
                        );
                    }
    ...
    attach(
        $(host)
            .empty()
            .html('<ul class="uk-pagination uk-flex-right"/>')
            .children("ul"),
        buttons
    );
};
...
function dtShiftUtility(dtTable, colsHide) {
    ...
    $(".dataTables_filter input")
        .off("keypress keyup search input paste cut")
        .typeWatch({
            allowSubmit: true,
            captureLength: 0,
