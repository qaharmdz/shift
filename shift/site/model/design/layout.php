...
class Layout extends Mvc\Model
{
    ...
    {
        $query = $this->db->get("SELECT * FROM " . DB_PREFIX . "layout_route WHERE '" . $this->db->escape($route) . "' LIKE route AND store_id = '" . (int)$this->config->get('system.setting.store_id') . "' ORDER BY route DESC LIMIT 1");
