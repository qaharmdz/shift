    ...
    public function index()
    {
        ...
        if ($this->request->is('POST') && $this->validate()) {
            if (!$this->request->has('query.module_id')) {
                $this->model_extension_module->addModule('slideshow', $this->request->get('post'));
            } else {
        ...
        if (!$this->request->has('query.module_id')) {
            $data['breadcrumbs'][] = array(
                ...
                'href' => $this->url->link('extension/module/slideshow', 'token=' . $this->session->get('token'), true)
            );
        } else {
            $data['breadcrumbs'][] = array(
                'text' => $this->language->get('heading_title'),
                'href' => $this->url->link('extension/module/slideshow', 'token=' . $this->session->get('token') . '&module_id=' . $this->request->get('query.module_id'), true)
            );
        }

        if (!$this->request->has('query.module_id')) {
            $data['action'] = $this->url->link('extension/module/slideshow', 'token=' . $this->session->get('token'), true);
        } else {
            $data['action'] = $this->url->link('extension/module/slideshow', 'token=' . $this->session->get('token') . '&module_id=' . $this->request->get('query.module_id'), true);
        }
