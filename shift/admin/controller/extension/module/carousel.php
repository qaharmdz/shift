    ...
    public function index()
    {
        ...
        if ($this->request->is('POST') && $this->validate()) {
            if (!$this->request->has('query.module_id')) {
                $this->model_extension_module->addModule('carousel', $this->request->get('post'));
            } else {
        ...
        if (!$this->request->has('query.module_id')) {
            $data['breadcrumbs'][] = array(
                ...
                'href' => $this->router->url('extension/module/carousel')
            );
        } else {
            $data['breadcrumbs'][] = array(
                'text' => $this->language->get('heading_title'),
                'href' => $this->router->url('extension/module/carousel' . '&module_id=' . $this->request->get('query.module_id'))
            );
        }

        if (!$this->request->has('query.module_id')) {
            $data['action'] = $this->router->url('extension/module/carousel');
        } else {
            $data['action'] = $this->router->url('extension/module/carousel' . '&module_id=' . $this->request->get('query.module_id'));
        }
