    ...
    public function index()
    {
        ...
        if (!$this->request->has('query.module_id')) {
            $data['breadcrumbs'][] = array(
                ...
                'href' => $this->url->link('extension/module/html', 'token=' . $this->session->get('token'), true)
            );
        } else {
            $data['breadcrumbs'][] = array(
                'text' => $this->language->get('heading_title'),
                'href' => $this->url->link('extension/module/html', 'token=' . $this->session->get('token') . '&module_id=' . $this->request->get('query.module_id'), true)
            );
        }

        if (!$this->request->has('query.module_id')) {
            $data['action'] = $this->url->link('extension/module/html', 'token=' . $this->session->get('token'), true);
        } else {
            $data['action'] = $this->url->link('extension/module/html', 'token=' . $this->session->get('token') . '&module_id=' . $this->request->get('query.module_id'), true);
        }
