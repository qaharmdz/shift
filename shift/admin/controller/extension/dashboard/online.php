...
class Online extends Mvc\Controller
{
    ...
    public function index()
    {
        ...
        if ($this->request->is('POST') && $this->validate()) {
            ...
            $this->response->redirect($this->router->url('extension/extension' . '&type=dashboard'));
        }
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->router->url('extension/extension' . '&type=dashboard')
        );

        $data['breadcrumbs'][] = array(
            'text' => $this->language->get('heading_title'),
            'href' => $this->router->url('extension/dashboard/online')
        );

        $data['action'] = $this->router->url('extension/dashboard/online');

        $data['cancel'] = $this->router->url('extension/extension' . '&type=dashboard');
    ...
    public function dashboard()
    {
        ...
        // Customers Online
        $data['total'] = 101;
        $data['online'] = $this->router->url('report/customer_online');
