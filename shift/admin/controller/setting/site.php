...
class Site extends Mvc\Controller
{
    ...
    protected function getForm()
    {
        ...
        if ($this->request->has('post.logo') && is_file(PATH_MEDIA . $this->request->get('post.logo'))) {
            $data['logo'] = $this->image->construct($this->request->get('post.logo'), 100, 100);
        } elseif (Arr::has($site_info, 'logo') && is_file(PATH_MEDIA . Arr::get($site_info, 'logo'))) {
            $data['logo'] = $this->image->construct(Arr::get($site_info, 'logo'), 100, 100);
        }

        $data['icon'] = $no_image_thumb;
        if ($this->request->has('post.icon') && is_file(PATH_MEDIA . $this->request->get('post.icon'))) {
            $data['icon'] = $this->image->construct($this->request->get('post.icon'), 100, 100);
        } elseif (Arr::has($site_info, 'icon') && is_file(PATH_MEDIA . Arr::get($site_info, 'icon'))) {
